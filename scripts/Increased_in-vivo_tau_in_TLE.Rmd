...
| `mCi.net`                       | Net injected dose of 18F-MK6240 radiotracer (in mCi).                       |
...
| `asm.names`                     | Names of anti-seizure medications taken.                                    |
...
| `ilae.class`                    | ILAE surgical outcome classification.                                       |
| `moca.Total.Score`              | Global cognitive score from the MoCA test.                                  |
...
| `Epi.Acc.E.per`                 | Percent accuracy in encoding trials.                                        |
| `Semantic`                      | Accuracy in the semantic decision task.                                     |
| `task.spatial`                  | Accuracy in the spatial memory task.                                        |
| `hip.ipsi`                      | Ipsilateral hippocampal volume (mm³ volume).                                |
...
names(mk.df)[names(mk.df) == "EpiTrack.Score.age.corrected"] <- "EpiTrack"
names(mk.df)[names(mk.df) == "Epi.Acc.D.per"] <- "Episodic"
names(mk.df)[names(mk.df) == "task.semantic"] <- "Semantic"
names(mk.df)[names(mk.df) == "task.spatial"] <- "Spatial"
...
mk.df %>%
  dplyr::select(
    group, age, mk6240.mean, mk6240.sig, mk6240.mean.ipsi, mk6240.mean.contra, mk6240.sig.ipsi, mk6240.sig.contra, mCi.net, mk6240.Tdiff,mk6240.mri.Tdiff,
    EpiTrack, Episodic,
    Semantic, Spatial, hip.ipsi, hip.cntr
  ) %>%
  tbl_summary(
    by = group,
    missing = "no",
    statistic = list(all_continuous() ~ "{mean}±{sd}"),
    type = list(mCi.net ~ "continuous")
...
mk.df %>%
  dplyr::select(group, mk6240.session, sex, handedness, age, mk6240.Tdiff, mCi.net) %>%
  tbl_strata(
    strata = group,
    .tbl_fun = ~ .x %>%
      tbl_summary(
...
# Create hippocampal atrophy vector (Normalized z-scored volume LESS than 2 std of controls)
mk.df$hip.atrophy <- ifelse(Zscore(mk.df$hip.ipsi[hc], mk.df$hip.ipsi)<=-1.5,1,0)

# Asymmetry z-score
As <- (mk.df$hip.ipsi - mk.df$hip.cntr) / (mk.df$hip.ipsi + mk.df$hip.cntr)
mk.df$assymetry <- ifelse(Zscore(As[hc],As)<=-1.5,1,0)

# Hippocampal sclerosis is defines as either atrophy or asymmetry
mk.df$hs <- as.numeric(mk.df$hip.atrophy |  mk.df$assymetry)

# Mean and Std
mk.df %>%
  subset(group=="Patient") %>% 
  dplyr::select(mk6240.session, duration, onset, origin, lateralization, hip.atrophy, assymetry, hs) %>%
...
mk.df$sc.pathlengh <- mk.df$sc.pathlengh/1e+15
mk.df$sc.clustecoef <- mk.df$sc.clustecoef*1e5
mk.df$sc.neighbors <- mk.df$sc.neighbors*1e3
mk.df$sc.efficiency <-mk.df$sc.efficiency*1e2
...
col.zscore <- c("EpiTrack", "Episodic", "Semantic", "Spatial", "hip.ipsi", "hip.cntr")
...
# Variables to correlate
corr.cli <- c("IEDs", "GTCSF.log", "Spatial")
...
```{r figure3b, fig.width=7, fig.height=6, warning=FALSE, echo=FALSE, message=FALSE}
# Save Figures as Correlation with behavior 1000 x 315 px

# Variables to correlate
corr.cli <- c("EpiTrack", "Episodic", "Semantic", "Spatial")
...
# Variables to correlate
corr.cli <- c("hip.ipsi", "hip.cntr","suvr.ipsi.hippocampus")
...
No significant interaction (`mk6240.Tdiff.yrs:groupHealthy`, p = 0.766) was found, suggesting that the rate of change in `mk6240.sig` over time is not significantly different between the Healthy and Patient groups.

-   **Healthy Group**: The estimated slope is `-0.0026` per year (95% CI: -0.034 to 0.0287). This suggests a slight decrease in `mk6240.sig` over time, but the change is small and not statistically significant.

-   **Patient Group**: The estimated slope is `0.0041` per year (95% CI: -0.0309 to 0.0391), indicating a slight increase in `mk6240.sig` over time, though this change is also not statistically significant.
...
# Variables to correlate
vars <- c("age", "duration", "onset","EpiTrack", "Episodic", "Semantic","hip.ipsi", "hip.cntr","mCi.net")
...
supplementary_clinical_table <- mk.df %>%
  ...
  # Replace NA in ilae.class with empty string
  mutate(ilae.class = ifelse(is.na(ilae.class), "", ilae.class)) %>%
  # Recode hip.atrophy 0=no, 1=yes
  mutate(hs = ifelse(hs == 0, "no", "yes")) %>%
  # Select relevant columns
  select(
    sex, age, handedness, Language,
...
# Save to CSV
write.csv(supplementary_clinical_table, "/Users/rcruces/Desktop/OSF_data_revision-2025/supplementary_clinical_table.csv", row.names = FALSE)
...
# Variables to correlate
vars <- c("mk6240.sig","EpiTrack", "Episodic", "Semantic", "hip.ipsi", "hip.cntr",
...
fit.full <- sem(model, data = mk.ses1, se = "bootstrap", bootstrap = 10000)
...
### Full SEM model with multiple mediators and outcomes and hippocampus
